...
class SherlockBaseTest(unittest.TestCase):
    def setUp(self):
        ...
        excluded_sites_path = os.path.join(os.path.dirname(os.path.realpath(sherlock_modul.__file__)), "tests/.excluded_sites")
        try:
    ...
    def username_check(self, username_list, site_list, exist_check=True):
        ...
        for username in username_list:
            results = sherlock_modul.sherlock(username,
                                        site_data,
                                        self.query_notify,
                                        tor=self.tor,
